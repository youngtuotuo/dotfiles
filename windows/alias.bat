...
DOSKEY vi=nvim $*
DOSKEY less=less -R $*
DOSKEY cd=if "$1" equ "~" ( cd /d %USERPROFILE% ) ELSE if "$1" equ " " ( cd /d %USERPROFILE% ) ELSE if "$1" equ "" ( cd /d %USERPROFILE% ) ELSE if "$1" equ "-" (cd /d %OLDPWD% && set OLDPWD="%cd%") ELSE ( cd /d $* && if not errorlevel 1 set OLDPWD="%cd%")
